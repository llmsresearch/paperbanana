"""Novita image generation provider — OpenAI-compatible endpoint for Novita AI models."""
...
class NovitaImageGen(ImageGenProvider):
    """Image generation using Novita's OpenAI-compatible API.

    Supports any image model available on Novita (DALL-E 3, Flux, etc.).
    Uses the OpenAI Python SDK with Novita's endpoint.
    ...
    def _get_client(self):
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "openai is required for the Novita provider. "
