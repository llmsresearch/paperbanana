...
def evaluate_poster_cmd(
    ...
    image-level venue compliance (dimensions / orientation / DPI)."""
    run_path = Path(run_dir).expanduser()
    poster_img = run_path / "poster.png"
    if not poster_img.is_file():
        console.print(f"[red]Error: no poster.png in {run_path}[/red]")
        raise typer.Exit(1)
    ...
    if context:
        ...
        grounding = run_path / "grounding.txt"
        if not grounding.is_file():
            console.print(
                f"[red]Error: no grounding.txt in {run_path}; pass --context explicitly[/red]"
            )
            raise typer.Exit(1)
        paper_context = grounding.read_text(encoding="utf-8")
